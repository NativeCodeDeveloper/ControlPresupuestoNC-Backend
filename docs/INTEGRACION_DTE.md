...
desde la descarga, no desde que se empieza a trabajar. El motor de firma/timbre (Fase 1B) ya está
listo; falta la Fase 1C para poder ejecutar este procedimiento.
...
- [ ] Solicitar CAF Tipo 39 (boletas) — bloque de 5 folios para el Set de Pruebas, **hacerlo recién
      cuando la Fase 1C esté lista** (ver §5)
- [ ] Solicitar CAF Tipo 33 (facturas) — bloque de 50, después de aprobar boletas
- [ ] Guardar archivos `.pfx` y CAF en lugar seguro (en el VPS, fuera del control de versiones)
...
- [ ] Generar y enviar el Set de Pruebas completo (§5) + RCOF
...
*Documento actualizado tras implementar y verificar el motor de firma/timbre nativo en Node
(Fase 1B). Próximo paso: Fase 1C (cliente SOAP) — requiere CAF real, respetar ventana de 24h.*
